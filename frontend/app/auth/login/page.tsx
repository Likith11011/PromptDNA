...
export default function LoginPage() {
  ...
  async function handleSubmit() {
    if (serverStatus === "waking") {
      setError("Server is still waking up. Please wait a moment.")
      return
    }
    setError("")
    setLoading(true)
    try {
      ...
      if (axiosErr.code === "ERR_NETWORK" || axiosErr.code === "ECONNABORTED") {
        setError("Cannot reach the server. Wait 30 seconds and try again.")
        setServerStatus("waking")
          ...
          <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 text-amber-700 text-sm">
            ...
            <div>
              ...
              <p className="text-amber-600 text-xs mt-0.5">
                This takes 30–60 seconds. Page updates automatically when ready.
              </p>
            </div>
          </div>
        )}
